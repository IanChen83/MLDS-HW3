...
SAMPLE      read_struct_examples(char *file, STRUCT_LEARN_PARM *sparm)
{
  ...
  LoadSequences(*seqs,"data/dnn_softmax_sub.ark");
  LoadAnswers(*seqs,"data/answer_map_sub.txt");
  SAMPLE   sample;  /* sample */
  EXAMPLE  *examples;
  long     n = seqs.size();       /* number of examples */

  
  examples=(EXAMPLE *)my_malloc(sizeof(EXAMPLE)*n);
  int i = 0;
  for(Sequences::iterator it = seqs->begin(); it != seqs->end(); ++it){
    examples[i].x.seq = it->second;
    int j = 0;
    Phoneme* pho = it->second->head;
    for(;pho != 0;pho = pho->next){
      examples[i].y.ans[j] = pho->ans;
      j++;
    }
    for(;j != 777; j++){
      examples[i].y.ans[j] = -1;
    }
    i++; 
  }  
