            ...
            STRUCT_LEARN_PARM *sparm)
{
  ...
  y.state_object = NULL;
  y.speaker = NULL;
  y.frame_num = MAX_ELEMENT_IN_OBSERVE;
  y_max.speaker = NULL;
             ...
             STRUCT_LEARN_PARM *sparm)
{
  ...
  */
  ...
  for(i=1;i<sm->svm_model->sv_num;i++) {
    for(v=sm->svm_model->supvec[i]->fvec;v;v=v->next) {
      fprintf(modelfl,"%.32g ",sm->svm_model->alpha[i]*v->factor);
...
void        write_label(FILE *fp, LABEL y)
{
  ...
  int i = 0;
  while(y.state_object[i] == 36 ) {  // skip the leading sil's
    i++;
  }
