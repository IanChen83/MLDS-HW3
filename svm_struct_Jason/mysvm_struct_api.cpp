...
/*  if (sentence_id == 500)
  ...
  //initial start_probability
  for (int i = 0; i < 48; i++){
    sparm->start_probability[i] = 0;
  }
  for (int i = 0; i < n; i++){
    sparm->start_probability[examples[i].y.state_object[0]] += 1;
  }
  for (int i = 0; i < 48; i++){
    sparm->start_probability[i] /= n;
  }
  //initial end_probability
  for (int i = 0; i < 48; i++){
    sparm->end_probability[i] = 0;
  }
  for (int i = 0; i < n; i++){
    sparm->end_probability[examples[i].y.state_object[examples[i].y.frame_num - 1]] += 1;
  }
  for (int i = 0; i < 48; i++){
    sparm->end_probability[i] /= n;
  }
            ...
            STRUCT_LEARN_PARM *sparm)
{
  ...
  y_max = viterbi_loss (x , y, sm, sparm, 0);
                 ...
                 STRUCT_LEARN_PARM *sparm)
{
  ...
  ybar = viterbi_loss(x , y, sm, sparm, 1);
  return(ybar);
}
             ...
             STRUCT_LEARN_PARM *sparm)
{
  ...
  // weight 
  for (i = 0; i < sm->sizePsi+1; i++) {(char*)malloc(sizeof(char)*20);
    fprintf(model, "%lf\n", sm->w[i]);
  }
  fprintf(model, "\n");
