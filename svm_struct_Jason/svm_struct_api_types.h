...
typedef struct pattern {
  ...
  float** observe_object; //  float observe_object[MAX_ELEMENT_IN_OBSERVE][OBSERVE_ELEMENT_DIM]
  int frame_num;
  char* speaker; // char speaker[20];
} PATTERN;

typedef struct label {
  /* this defines the y-part (the label) of a training example,
     e.g. the parse tree of the corresponding sentence. */
  int* state_object; // int state_object[MAX_ELEMENT_IN_OBSERVE]
  int frame_num;
  char* speaker; // char speaker[20]
} LABEL;
